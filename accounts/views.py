...
# Activation user section (Decode block)
...
def resetPassword(request):
    if request.method == 'POST':
        ...
        if password == confirm_password:
            ...
            user.set_password(password)  # Save into hash format
            user.save()
...
def my_orders(request):
    orders = Order.objects.filter(user=request.user, is_ordered=True).order_by('-created_at')  # The hyphen is used to retrive the result in descending order
    context = {
        'orders': orders
    }
...
# Edit profile validation form
@login_required(login_url='login')
def edit_profile(request):
    userprofile = get_object_or_404(UserProfile, user=request.user)

    if request.method == 'POST':
        user_form = UserForm(request.POST, instance=request.user)  # The instance ensure the unique profile
        profile_form = UserProfileForm(request.POST, request.FILES, instance=userprofile)  # request.FILES is required for the file uploaded
    ...
    if request.method == 'POST':
        ...
        if new_password == confirm_password:
            ...
            if success:
                ...
                user.save()
                # auth.logout(request) could be called if you would like to log out

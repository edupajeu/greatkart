...
def add_cart(request, product_id):
    ...
    if current_user.is_authenticated:
        ...
        if is_cart_exists:
            cart_item = CartItem.objects.filter(product=product, user=current_user)  # Get the Car item Object
...
def remove_cart_item(request, product_id, cart_item_id):
    ...
    if request.user.is_authenticated:
        ...
        cart = Cart.objects.get(cart_id=_cart_id(request))
        CartItem.objects.get(product=product, cart=cart, id=cart_item_id)
    cart_item.delete()
    return redirect('cart')
...
def cart(request, total=0, quantity=0, cart_items=None):
    try:
        tax = 0
        grand_total = 0
        if request.user.is_authenticated:  # check if user is authenticated to be assigned item counter
        ...
        tax = (10 * total) / 100  # Tax and grand total maths
